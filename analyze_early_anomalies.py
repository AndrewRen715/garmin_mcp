...
def analyze_activity_splits(garmin, activity_id, activity_name, max_hr, date):
    """分析活动的split数据"""
    try:
        # 获取活动的split数据
...
def analyze_early_anomalies():
    ...
    # 按日期排序，选择最早的几个活动（最后5个）
    early_anomalies = sorted(anomalies_data['anomalies'], key=lambda x: x['date'])
    # 选择最早的5个活动
    early_anomalies = early_anomalies[:5]
    
    print(f"分析最早的5个心率异常活动:\n")
    for anomaly in early_anomalies:
        print(f"- {anomaly['date']}: {anomaly['name']} (ID: {anomaly['id']}, 最大心率: {anomaly['max_hr']} BPM)")
    print()
    ...
    for anomaly in early_anomalies:
        ...
        print(f"分析活动: {activity_name} (ID: {activity_id}, 日期: {activity_date}, 最大心率: {max_hr} BPM)")
        
        # 分析split数据
        result = analyze_activity_splits(garmin, activity_id, activity_name, max_hr, activity_date)
    ...
    # 保存分析结果
    if analysis_results:
        output_file = "early_anomalies_analysis.json"
...
def generate_report(results):
    """生成分析报告"""
    report_file = "early_anomalies_analysis_report.md"
    
    with open(report_file, 'w', encoding='utf-8') as f:
        f.write("# 早期心率异常Split分析报告\n\n")
