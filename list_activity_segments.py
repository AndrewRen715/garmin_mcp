#!/usr/bin/env python3
"""
List activity segments for the 2024-10-08 trail run activity
"""
...
def list_activity_segments():
    """List activity segments for the trail run"""
    print("Listing activity segments for 2024-10-08 trail run...")
    ...
    for km in range(total_km):
        ...
        # Calculate time for this kilometer (based on average pace with elevation adjustment)
        # For every 100m of elevation gain, add 1 min/km to pace
        elevation_adjustment = (climb / 100) * 1.0
        km_pace = average_pace + elevation_adjustment
    ...
    if total_distance_km > total_km:
        ...
        km_pace = average_pace + elevation_adjustment
        km_time = distance * km_pace
        cumulative_time = total_time_minutes  # Ensure total time matches
    ...
    print("Activity Segments (Per Kilometer):")
    print("-" * 150)
