...
def analyze_hr_anomaly(garmin_client, activity_id, max_hr):
    ...
    try:
        ...
        # 获取心率数据
        hr_data = garmin_client.get_activity_heart_rates(activity_id)
...
def main():
    ...
    """
    print("=" * 80)
    print("Garmin Connect 心率异常详细分析工具")
    print("=" * 80)
    ...
    try:
        with open('hr_anomaly_details.json', 'w', encoding='utf-8') as f:
            json.dump(analysis_data, f, indent=2, ensure_ascii=False)
        print(f"\n分析结果已保存到: hr_anomaly_details.json")
    except Exception as e:
        print(f"保存分析结果失败: {e}")
