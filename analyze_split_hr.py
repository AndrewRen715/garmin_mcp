...
def analyze_activity_splits(garmin_client, activity_id, activity_name, max_hr):
    """
    分析活动的split数据
...
def main():
    ...
    """
    print("=" * 80)
    print("Garmin Connect 活动Split心率分析工具")
    print("=" * 80)
    print("分析活动的split数据，计算前一个split的平均心率")
    print("=" * 80)
    ...
    try:
        ...
        print("\n开始分析活动...")
        print("=" * 80)
        print(f"{'活动ID':<12} {'名称':<20} {'最大心率':<8} {'最大心率Split':<12} {'前一个Split平均心率':<15} {'状态':<10}")
        print("=" * 80)
        ...
        for activity in activities:
            ...
            print(f"分析活动: {activity_id} ({activity_name[:18]})")
            
            result = analyze_activity_splits(garmin_client, activity_id, activity_name, max_hr)
            ...
            print(f"{str(activity_id):<12} {activity_name[:18]:<20} {max_hr:<8} {max_hr_split:<12} {prev_avg_hr:<15} {status:<10}")
        ...
        try:
            with open('split_hr_analysis.json', 'w', encoding='utf-8') as f:
                json.dump(analysis_data, f, indent=2, ensure_ascii=False)
            print(f"\n分析结果已保存到: split_hr_analysis.json")
        except Exception as e:
            print(f"保存分析结果失败: {e}")
