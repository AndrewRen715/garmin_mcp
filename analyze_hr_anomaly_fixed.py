...
def analyze_hr_anomaly(garmin_client, activity_id, max_hr):
    """
    分析单个心率异常活动
    ...
    try:
        ...
        # 获取心率数据
        try:
            # 尝试使用get_heart_rates方法
            hr_data = garmin_client.get_heart_rates(activity_id)
        except Exception as e:
            print(f"get_heart_rates失败: {e}")
            # 尝试使用get_activity_hr_in_timezones方法
            try:
                hr_data = garmin_client.get_activity_hr_in_timezones(activity_id)
            except Exception as e2:
                print(f"get_activity_hr_in_timezones也失败: {e2}")
                return {
                    'success': False,
                    'error': '无法获取心率数据'
                }
...
def main():
    ...
    """
    print("=" * 80)
    print("Garmin Connect 心率异常详细分析工具 (修复版)")
    print("=" * 80)
    ...
    try:
        with open('hr_anomaly_details_fixed.json', 'w', encoding='utf-8') as f:
            json.dump(analysis_data, f, indent=2, ensure_ascii=False)
        print(f"\n分析结果已保存到: hr_anomaly_details_fixed.json")
    except Exception as e:
        print(f"保存分析结果失败: {e}")
